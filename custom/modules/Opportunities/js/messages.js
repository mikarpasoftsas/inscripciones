var messageConfig = {
    ...
    form: {
        input: {'placeholder': 'Ingresar mensaje ...'},
        button: {'label': 'Enviar'},
...
$(document).ready(function () {
    createChatBox(function () {
        loadMoreData(function(totalRecords){
            // if first call records not found destroy chatbox.
            if(totalRecords == 0){
                $('.chat-box').remove();
            }
        });
...
function createChatBox(callback) {
    $('body').append('<div class="chat-box">' +
            ...
            '<textarea required="required" name="message" placeholder="' + messageConfig.form.input.placeholder + '" class="form-control"></textarea>' +
